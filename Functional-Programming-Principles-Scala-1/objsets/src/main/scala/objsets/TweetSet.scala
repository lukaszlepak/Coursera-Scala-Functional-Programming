...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter(p: Tweet => Boolean): TweetSet = {
    val t = if(p(elem)) new NonEmpty(elem, new Empty, new Empty) else new Empty
    left.filterAcc(p, right.filterAcc(p, t))
  }
  ...
  def mostRetweeted: Tweet = {
    val t = try {
      val t1 = left.mostRetweeted
      if (elem.retweets > t1.retweets) elem else t1
    } catch {
      case e: NoSuchElementException => elem
    }
    try {
      val t2 = right.mostRetweeted
  ...
  def descendingByRetweet: TweetList = {
    def loop(tweetSet: TweetSet): TweetList = {
      try {
        val t1 = tweetSet.mostRetweeted
        new Cons(t1, loop(tweetSet.remove(t1)))
      } catch {
