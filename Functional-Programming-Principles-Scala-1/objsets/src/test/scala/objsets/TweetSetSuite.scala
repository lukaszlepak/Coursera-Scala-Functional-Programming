...
class TweetSetSuite {
  ...
  @Test def `mostRetweeted`: Unit =
    new TestSets {
      val t = new Tweet("z", "zzz", 40)
      val set6 = set5.incl(t)
      assertEquals(t, set6.mostRetweeted)
    }

  @Test def `descending: set5`: Unit =
    new TestSets {
      val trends = set5.descendingByRetweet
      assert(!trends.isEmpty, " NA EMPTY")
      assert(trends.head.user == "a" || trends.head.user == "b", trends.head.user)
      assert(trends.tail.head.user == "a" || trends.tail.head.user == "b", trends.head.user)
    }
